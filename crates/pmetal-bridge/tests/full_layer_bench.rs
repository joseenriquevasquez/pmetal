...
fn real_wt(in_dim: i32, out_dim: i32, dtype: i32) -> InlineArray {
    ...
    let mut w = InlineArray::arange(n, dtype).reshape(&[out_dim, in_dim]).t();
    w.eval();
    w
}
...
fn full_layer_forward() {
    let dt = 11; // bf16
    let h = 1024i32;
    let nk = 16i32; let nv = 16i32; let dk = 128i32; let dv = 128i32;
    let cd = 6144i32;
    let ck = 4i32;
    let n_heads = 8i32; let n_kv = 2i32; let head_dim = 256i32;
    let inter = 3584i32;

    // Weights
    struct LW {
        ln1_w: InlineArray, ln2_w: InlineArray,
        gate_wt: InlineArray, up_wt: InlineArray, down_wt: InlineArray,
        is_gdn: bool,
        // GDN
        qkvz_wt: Option<InlineArray>, ba_wt: Option<InlineArray>,
        conv_w: Option<InlineArray>,
        q_nw: Option<InlineArray>, k_nw: Option<InlineArray>,
        a_log: Option<InlineArray>, dt_bias: Option<InlineArray>,
        out_wt: Option<InlineArray>, norm_w: Option<InlineArray>,
        // Attn
        q_wt: Option<InlineArray>, k_wt: Option<InlineArray>,
        v_wt: Option<InlineArray>, o_wt: Option<InlineArray>,
    }
    ...
    for i in 0..24 {
        ...
        let mut lw = LW {
            ...
            is_gdn,
            qkvz_wt: None, ba_wt: None, conv_w: None,
            q_nw: None, k_nw: None, a_log: None, dt_bias: None,
            out_wt: None, norm_w: None,
    ...
    let simplified_step = |tok: i32| -> InlineArray {
        ...
        hidden.rms_norm(Some(&final_ln_w), 1e-6).matmul(&embed_w.t())
    };

    // ── Test B: With GDN conv + kernel (adds ~480 dispatches) ──
    let mut gdn_states: Vec<InlineArray> = (0..18).map(|_| InlineArray::zeros(&[1, nv, dv, dk], 10)).collect();
    let mut conv_states: Vec<InlineArray> = (0..18).map(|_| InlineArray::zeros(&[1, ck-1, cd], dt)).collect();

    let full_step = |tok: i32, gdn_states: &mut Vec<InlineArray>, conv_states: &mut Vec<InlineArray>| -> InlineArray {
        let t = InlineArray::from_i32(tok).reshape(&[1, 1]);
        let mut hidden = embed_w.take_axis(&t.squeeze(0), 0).reshape(&[1, 1, h]);
        ...
        for lw in &layers {
            ...
            let r = if lw.is_gdn {
                ...
                let kd = nk*dk;
                let q = conv_out.slice(&[0, 0, 0], &[1, 1, kd]).reshape(&[1, 1, nk, dk]);
                let k = conv_out.slice(&[0, 0, kd], &[1, 1, kd*2]).reshape(&[1, 1, nk, dk]);
                let v = conv_out.slice(&[0, 0, kd*2], &[1, 1, cd]).reshape(&[1, 1, nv, dv]);
                ...
                let g = InlineArray::fused_compute_g(lw.a_log.as_ref().unwrap(), &a_parts, lw.dt_bias.as_ref().unwrap());
                ...
                out.reshape(&[1, 1, nv*dv]).matmul(lw.out_wt.as_ref().unwrap())
            } else {
                let _q = normed.matmul(lw.q_wt.as_ref().unwrap());
                let _k = normed.matmul(lw.k_wt.as_ref().unwrap());
                let _v = normed.matmul(lw.v_wt.as_ref().unwrap());
    ...
    // Warmup simplified
    for i in 0..5 { let mut r = simplified_step(i); r.eval(); }
    ...
    // Warmup full
    for i in 0..5 { let mut r = full_step(i, &mut gdn_states, &mut conv_states); r.eval(); }
    ...
    eprintln!("GDN overhead: {:.2}ms ({:.1}x)", avg_b - avg_a, avg_b / avg_a);
}
