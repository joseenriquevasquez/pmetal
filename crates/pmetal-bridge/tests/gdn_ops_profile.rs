...
fn bench(label: &str, mut setup: impl FnMut() -> InlineArray, iters: usize) {
    for _ in 0..5 { let mut r = setup(); r.eval(); }
...
fn bench2(label: &str, mut setup: impl FnMut() -> (InlineArray, InlineArray), iters: usize) {
    for _ in 0..5 { let (mut a, mut b) = setup(); a.eval(); b.eval(); }
    let mut times = Vec::new();
    for _ in 0..iters {
...
fn profile_gdn_specific_ops() {
    ...
    bench("conv1d depthwise (groups=6144)", || {
    ...
    bench2("GDN metal_step (nv=16, dk=128, dv=128)", || {
    ...
    bench("SDPA (8 heads, seq=50, hd=256)", || {
    ...
    bench("fused_swiglu", || {
        InlineArray::fused_swiglu(&gate, &up)
    }, 30);

    // ── 18x GDN layers total (just conv + kernel) ──
    bench("18x (conv1d + gdn_kernel)", || {
